...
class Loader_batch(Loader):
    def __init__(self,
        ...
        dataset = Dataset_(n_features=data.shape[1])
        self.dataset = dataset
    ...
    def _create_windows_batch(self):
        ...
        for Y in self.data:
            ...
            self.mask_windows.append(t.zeros_like(Y))

        self.Y_windows = t.stack(self.Y_windows, dim=0)
        self.mask_windows = t.stack(self.mask_windows, dim=0)
        self.X_windows = None
        self.n_idxs = len(self.Y_windows)
        self.n_batch_in_epochs = int(np.ceil(self.n_idxs / self.batch_size))
